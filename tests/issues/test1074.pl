...
% the file is consulted, which aborts the load, so those cases live in
% quads_malformed.pl; a '$quad' fact can also be asserted by hand, and
% library(quads) makes the same check on those.

main :-
	use_module(library(quads)),
	assertz('$quad'((X=1), ['X'=X], (1 = X), 'hand-written.pl', 1)),
	assertz('$quad'((Y=1), ['Y'=Y], (Y = 1, Y = 2), 'hand-written.pl', 2)),
	assertz('$quad'((Z=1), ['Z'=Z], (Z = 1), 'hand-written.pl', 3)),
	run_quads.
