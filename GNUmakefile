...
# The embedder demo doubles as the smoke test for libtrealla.a, so it is
# built by default and run by `make misc`. Skipped where this build is
# producing something other than a native binary: under WASI the LDFLAGS
# already carry '-o tpl.wasm', and a Windows cross-build has no native
# loader to run it with.
...
cosmo:
	$(MAKE) clean
	$(MAKE) CC=cosmocc NOTHREADS=1 ISOCLINE=1 NOSSL=1 NOFFI=1 NOPIC=1
