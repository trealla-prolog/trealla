...
static bool bif_sys_cleanup_if_det_1(query *q)
{
	...
	cell *c = deref(q, ch->st.instr, ch->st.cur_fp);
...
bool call_check(query *q, cell *p1, bool *status, bool calln)
{
	...
	if ((p1->arity == 2) && is_builtin(p1)
		...
		*status = throw_error(q, save_p1, q->st.cur_fp, "type_error", "callable");
		return false;
	}
...
static bool bif_iso_call_n(query *q)
{
	...
	cell *tmp = prepare_call(q, CALL_NOSKIP, tmp2, q->st.cur_fp, 3);
	checked(tmp);
...
static bool bif_ignore_1(query *q)
{
	...
	cell *tmp = prepare_call(q, CALL_NOSKIP, p1, q->st.cur_fp, 4);
	checked(tmp);
	tmp->flags &= ~FLAG_INTERNED_TAIL_CALL;
	pl_idx num_cells = p1->num_cells;
	make_instr(tmp+num_cells++, g_cut_s, bif_iso_cut_0, 0, 0);
...
{
	cell *tmp = prepare_call(q, CALL_NOSKIP, p1, q->st.cur_fp, 3+p2->num_cells+2);
	checked(tmp);
	pl_idx num_cells = p1->num_cells;
	make_instr(tmp+num_cells++, g_cut_s, bif_iso_cut_0, 0, 0);
	make_instr(tmp+num_cells++, g_sys_drop_barrier_s, bif_sys_drop_barrier_1, 1, 1);
	make_uint(tmp+num_cells++, q->cp);
	num_cells += dup_cells_by_ref(tmp+num_cells, p2, q->st.cur_fp, p2->num_cells);
	make_instr(tmp+num_cells++, g_true_s, bif_iso_true_0, 0, 0); // Why???
	make_call(q, tmp+num_cells);
...
{
	cell *tmp = prepare_call(q, CALL_NOSKIP, p1, q->st.cur_fp, 4+p2->num_cells+2);
	checked(tmp);
	pl_idx num_cells = p1->num_cells;
...
static bool bif_shift_1(query *q)
{
	...
	dup_cells_by_ref(tmp2+1, next, q->st.cur_fp, next->num_cells);
	q->cont = tmp2;
	q->cont_ctx = q->st.cur_fp;
...
static cell *parse_to_heap(query *q, const char *src)
{
	...
	cell *tmp = clone_term_to_heap(q, p2->cl->cells, q->st.cur_fp);
