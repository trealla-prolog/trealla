...
static void db_log(query *q, rule *r, enum log_type l)
{
	...
	switch(l) {
	case LOG_ASSERTA:
		dst = print_term_to_strbuf(q, r->cl.cells, q->st.curr_ctx, 1);
...
bool do_abolish(query *q, cell *c_orig, cell *c_pi, bool hard)
{
	...
	if (!pr->is_dynamic)
		return throw_error(q, c_orig, q->st.curr_ctx, "permission_error", "modify,static_procedure");
...
static bool bif_iso_asserta_1(query *q)
{
	...
	if (is_var(head))
		return throw_error(q, head, q->st.curr_ctx, "instantiation_error", "args_not_sufficiently_instantiated");

	if (!is_interned(head) && !is_cstring(head))
		return throw_error(q, head, q->st.curr_ctx, "type_error", "callable");

	bool found = false;

	if (get_builtin_term(q->st.m, head, &found, NULL), found) {
		if (!GET_OP(head)) {
			return throw_error(q, head, q->st.curr_ctx, "permission_error", "modify,static_procedure");
		}
	}
	...
	if (body && ((tmp2 = check_body_callable(body)) != NULL)) {
		return throw_error(q, tmp2, q->st.curr_ctx, "type_error", "callable");
	}
	...
	if (!r) {
		h = copy_term_to_heap(q, h, q->st.curr_ctx, false);
		return throw_error(q, h, q->st.curr_ctx, "permission_error", "modify,static_procedure");
	}
...
static bool do_asserta_2(query *q)
{
	...
	if (!is_var(p2)) {
		...
		unify(q, p2, p2_ctx, &tmp2, q->st.curr_ctx);
		unshare_cell(&tmp2);
	}
...
static bool bif_instance_2(query *q)
{
	...
	rule *r = find_in_db(q->st.m, &u);
	CHECKED(r);
	return unify(q, p2, p2_ctx, r->cl.cells, q->st.curr_ctx);
}
...
static bool bif_sys_retract_on_backtrack_1(query *q)
{
	...
	CHECKED(b->ptr2);
	cell c, v;
	make_ref(&c, var_num, q->st.curr_ctx);
	make_dbref(&v, b);
	return unify(q, &c, q->st.curr_ctx, &v, q->st.curr_ctx);
}
...
static bool do_dump_term(query *q, cell *p1x, pl_ctx p1x_ctx, cell *p1, pl_ctx p1_ctx, bool deref, int depth)
{
	...
	for (unsigned i = 0; i < p1->num_cells; i++, tmp++) {
		...
		if (is_var(tmp) && deref) {
			...
			if (e->c.val_attrs && 0) {
				printf("\n");
				do_dump_term(q, p1x, p1x_ctx, e->c.val_attrs, q->st.curr_ctx, deref, depth+1);
				continue;
			}
		}
