...
bool needs_quoting(module *m, const char *src, int srclen)
{
    ...
    while (slen > 0) {
        ...
        if (((ch < 256) && strchr(g_solo, ch)) || iswspace(ch) || iswblank(ch) || (ch == 0xa0) ||
            (ch == 0x85) || (ch == 0x2007) || (ch == 0x202f))
            return true;
...
static bool op_needs_quoting(module *m, const char *src, int srclen)
{
    ...
    if (match_op(m, src, NULL, false))
        return strchr(src, ' ') || strchr(src, '\'') || strchr(src, '\"') || !strcmp(src, "(") ||
               !strcmp(src, ")") || !strcmp(src, "[") || !strcmp(src, "]") || !strcmp(src, "{") ||
               !strcmp(src, "}");
...
char *formatted(const char *src, int srclen, bool dq, bool json)
{
    ...
    while (srclen > 0) {
        ...
        srclen -= lench;
        const char *ptr =
            (lench == 1) && (ch != ' ') && (ch != '\e') ? strchr(g_escapes, ch) : NULL;
...
static const char *varformat(char *tmpbuf, unsigned long long num, bool listing)
{
    ...
    if ((num / 26) > 0)
        dst += sprintf(dst, "%" PRIu64 "", (int64_t)(num / 26));
    return tmpbuf;
}
...
static void print_variable(query *q, cell *c, pl_ctx c_ctx, bool running)
{
    ...
    pl_idx slot_nbr = running ? (pl_idx)(get_actual_slot_num(q, f, c->var_num)) : c->var_num;
...
static bool dump_variable(query *q, cell *c, pl_ctx c_ctx, bool running)
{
    ...
    while (is_iso_list(l)) {
        ...
        pl_idx slot_nbr = running ? (pl_idx)(get_actual_slot_num(q, f, v->var_num)) : v->var_num;
...
static void print_string_canonical(query *q, cell *c, pl_ctx c_ctx, int running, bool cons,
                                   unsigned depth)
{
    unsigned cnt = 1;
    LIST_HANDLER(c);
...
static void print_string_list(query *q, cell *c, pl_ctx c_ctx, int running, bool cons,
                              unsigned depth)
{
    LIST_HANDLER(c);
    if (!cons) {
...
static bool print_term_to_buf_(query *q, cell *c, pl_ctx c_ctx, int running, int cons,
                               unsigned print_depth, unsigned depth, visit *);

static void print_iso_list(query *q, cell *c, pl_ctx c_ctx, int running, bool cons,
    ...
    while (is_iso_list(c)) {
        ...
        if (q->do_dump_vars && is_var(save_head) && 0 && is_cyclic_term(q, head, c_ctx)) {
            ...
            if ((q->portray_vars || q->do_dump_vars) &&
                ((unsigned)q->dump_var_num != (unsigned)-1)) {
                SB_sprintf(q->sb, "%s", GET_POOL(q, q->top->vartab.off[q->dump_var_num]));
            } else {
                SB_sprintf(q->sb, "%s", "...");
            }
        ...
        if (is_interned(tail) && !is_compound(tail)) {
            ...
            if (strcmp(src, "[]")) {
                ...
                print_term_to_buf_(q, tail, tail_ctx, running, true, depth + 1, depth + 1, visited);
                   ...
                   possible_chars &&
                   (scan_is_chars_list2(q, tail, tail_ctx, false, &has_vars, &is_partial, NULL) >
                    0) &&
                   !is_partial) {
...
static bool print_interned(query *q, cell *c, pl_ctx c_ctx, bool running, unsigned depth,
                           visit *visited)
{
    // ATOM / COMPOUND
    ...
    if (q->ignore_ops || !is_op || !c->arity) {
        ...
        if (q->quoted < 0)
            quote = 0;
        if ((c->arity == 1) && is_interned(c) && !strcmp(src, "{}"))
            braces = 1;
        cell *c1 = c->arity && running ? deref(q, FIRST_ARG(c), c_ctx) : NULL;

        if (running && is_interned(c) && c->arity && q->numbervars && (c->val_off == g_sys_var_s) &&
            c1 && is_integer(c1) && (get_smallint(c1) >= 0)) {
    ...
    if (is_op_postfix) {
        ...
        unsigned lhs_pri =
            is_interned(lhs) ? match_op(q->st.m, C_STR(q, lhs), &lhs_specifier, lhs->arity) : 0;
        ...
        if ((c->val_off == g_plus_s) && is_op_lhs)
            space = true;
        int ch = peek_char_utf8(src);
        if (iswalpha(ch))
            space = true;
        if (lhs_pri > my_priority) {
            parens = true;
            space = false;
        }
        ...
        // if (q->last_thing != WAS_SPACE) { SB_sprintf(q->sb, "%s", " "); q->last_thing =
        // WAS_SPACE; }
        else
            q->last_thing = WAS_OTHER;
        return true;
    ...
    if (is_op_prefix) {
        ...
        unsigned rhs_pri =
            is_interned(rhs) ? match_op(q->st.m, C_STR(q, rhs), NULL, rhs->arity) : 0;
        ...
        if ((c->val_off == g_plus_s) && is_op_rhs)
            space = true;
        int ch = peek_char_utf8(src);
        if (iswalpha(ch))
            space = true;
        if (/*is_op_rhs ||*/ is_negative(rhs) || is_float(rhs))
            space = true;
        if (is_interned(rhs) && !iswalpha(peek_char_utf8(rhs_src)) && !is_op(rhs))
            space = true;

        bool parens = false;
        if ((!strcmp(src, ":-") || !strcmp(src, "?-")) && (rhs_pri >= my_priority))
            parens = true;
        if (!strcmp(src, "+") && (is_infix(rhs) || is_postfix(rhs)))
            parens = true;
        if (rhs_pri > my_priority)
            parens = true;
        if ((rhs_pri > 0) && !rhs->arity)
            parens = true;
        // if (my_priority && (rhs_pri == my_priority) && strcmp(src, "-") && strcmp(src, "+"))
        // parens = true;
        if (!strcmp(src, "-") && (rhs_pri == my_priority) && (rhs->arity > 1))
            parens = true;
        if ((c->val_off == g_minus_s) && is_number(rhs) && !is_negative(rhs))
            parens = true;
        if ((c->val_off == g_minus_s) && match_op(q->st.m, C_STR(q, rhs), NULL, true) &&
            !rhs->arity)
            parens = true;
        if ((c->val_off == g_plus_s) && match_op(q->st.m, C_STR(q, rhs), NULL, true) && !rhs->arity)
            parens = true;
        ...
        if (is_interned(rhs) && !rhs->arity && !parens) {
            if (!iswalnum(peek_char_utf8(rhs_src)) && strcmp(rhs_src, "[]") &&
                strcmp(rhs_src, "{}"))
                space = 1;
        }
    ...
    if (op_needs_quoting(q->st.m, src, src_len))
        quote = 1;
    ...
    unsigned lhs_pri_1 =
        is_interned(lhs) ? match_op(q->st.m, C_STR(q, lhs), &lhs_specifier, lhs->arity) : 0;
    unsigned lhs_pri_2 = is_interned(lhs) && !lhs->arity
                             ? match_op(q->st.m, C_STR(q, lhs), &lhs_specifier, true)
                             : 0;
    bool lhs_postfix = (lhs->arity == 1) && IS_POSTFIX(lhs_specifier);

    bool lhs_parens = lhs_pri_1 >= my_priority;
    // if (lhs_postfix) lhs_parens = true;
    if ((lhs_pri_1 == my_priority) && is_op_yfx)
        lhs_parens = false;
    if (lhs_pri_2 > 0)
        lhs_parens = true;
    if (is_compound(lhs) && (lhs_pri_1 <= my_priority) && (lhs->val_off == g_plus_s)) {
        lhs_parens = false;
    }
    ...
    if (is_interned(lhs) && !lhs->arity && !lhs_parens) {
        const char *lhs_src = C_STR(q, lhs);
        if (!iswalpha(peek_char_utf8(lhs_src)) && !iswdigit(peek_char_utf8(lhs_src)) &&
            (peek_char_utf8(lhs_src) != '$') && strcmp(src, ",") && strcmp(src, ";") &&
    ...
    int ch = peek_char_utf8(src);
    bool is_symbol = !needs_quoting(q->st.m, src, src_len) && !iswalpha(ch) && strcmp(src, ",") &&
                     strcmp(src, ";") && strcmp(src, "[]") && strcmp(src, "{}") && !q->parens;
    ...
    unsigned rhs_pri_2 =
        is_interned(rhs) && !rhs->arity ? match_op(q->st.m, C_STR(q, rhs), NULL, true) : 0;
    ...
    bool rhs_is_symbol = is_interned(rhs) && !rhs->arity && !iswalpha(peek_char_utf8(rhs_src)) &&
                         !needs_quoting(q->st.m, C_STR(q, rhs), C_STRLEN(q, rhs)) &&
                         strcmp(C_STR(q, rhs), "[]") && strcmp(C_STR(q, rhs), "{}") && !rhs_parens;

    if (rhs_is_symbol && strcmp(C_STR(q, rhs), "[]") && strcmp(C_STR(q, rhs), "{}") &&
        strcmp(C_STR(q, rhs), "!"))
        space = true;
                               ...
                               unsigned print_depth, unsigned depth, visit *visited)
{
    ...
    if (!is_chars_list && running && possible_chars &&
        ...
        is_chars_list += q->st.m->flags.double_quote_chars &&
                         scan_is_chars_list2(q, c, c_ctx, false, &has_var, &is_partial, &v);
