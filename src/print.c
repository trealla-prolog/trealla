...
static void print_iso_list(query *q, cell *c, pl_idx c_ctx, int running, bool cons, unsigned print_depth, unsigned depth)
{
	...
	while (is_iso_list(c)) {
		...
		slot *e = NULL;
		uint64_t save_vgen1 = 0;
		int both = 0;
		if (running) DEREF_CHECKED(any1, both, save_vgen1, e, e->vgen1, head, head_ctx, q->print_vgen)
		...
		if (running) DEREF_CHECKED(any2, both, e->save_vgen1, e, e->vgen1, tail, tail_ctx, q->print_vgen);
...
static bool print_term_to_buf_(query *q, cell *c, pl_idx c_ctx, int running, int cons, unsigned print_depth, unsigned depth)
{
	...
	if (q->ignore_ops || !IS_OP(c) || !c->arity) {
		...
		if (is_compound(c) && !is_string(c)) {
			...
			for (c++; arity--; c += c->nbr_cells) {
				...
				uint64_t save_vgen1 = 0;
				int both = 0;
				if (running) DEREF_CHECKED(any, both, save_vgen1, e, e->vgen1, tmp, tmp_ctx, q->print_vgen);
	...
	if (is_postfix(c)) {
		...
		bool any = false;
		int both = 0;
		if (running) DEREF_CHECKED(any, both, save_vgen1, e, e->vgen1, lhs, lhs_ctx, q->print_vgen);
	...
	if (is_prefix(c)) {
		...
		bool any = false;
		int both = 0;
		if (running) DEREF_CHECKED(any, both, save_vgen1, e, e->vgen1, rhs, rhs_ctx, q->print_vgen);
