...
static void compile_term(predicate *pr, clause *cl, cell **dst, cell **src)
{
    ...
    if (((*src)->val_off == g_disjunction_s) && ((*src)->arity == 2) && is_callable(c) &&
        c->bif_ptr && (c->arity == 2) && (c->bif_ptr->fn == bif_iso_if_then_2)) {
        *src += 2;
    ...
    if (((*src)->val_off == g_disjunction_s) && ((*src)->arity == 2) && is_callable(c) &&
        c->bif_ptr && (c->arity == 2) && (c->bif_ptr->fn == bif_soft_if_then_2)) {
        *src += 2;
        unsigned var_num = cl->num_vars++;
        cell *save_dst1 = *dst;
        make_instr((*dst)++, g_sys_succeed_on_retry_s, bif_sys_succeed_on_retry_2, 2, 2);
