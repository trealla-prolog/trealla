...
USE_RESULT bool fn_sys_register_function_4(query *q)
{
	...
	while (is_iso_list(l) && (idx < MAX_FFI_ARGS)) {
		...
		if (is_interned(h)) {
			...
			else if (!strcmp(src, "val_ffi_uint"))
			...
			else if (!strcmp(src, "val_ffi_sint"))
	...
	while (is_iso_list(l) && (idx < MAX_FFI_ARGS)) {
		...
		if (is_interned(h)) {
			...
			if (!strcmp(src, "uint8"))
				...
				arg_types[idx++] = TAG_UINT;
			else if (!strcmp(src, "-") && !strcmp(C_STR(m, h+1), "val_ffi_uint"))
				...
				arg_types[idx++] = TAG_INT;
			else if (!strcmp(src, "-") && !strcmp(C_STR(m, h+1), "val_ffi_sint"))
...
bool wrapper_for_predicate(query *q, builtins *ptr)
{
	...
	if (ptr->ret_type == TAG_UINT8) {
		...
		make_ptr(&tmp, r.val_ffi_p);
		bool ok = unify(q, c, c_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		if (ok != true) return ok;
	} else if (ptr->ret_type == TAG_CSTR) {
		check_heap_error(make_cstring(&tmp, r.val_ffi_s));
		free(r.val_ffi_s);
		bool ok = unify(q, c, c_ctx, &tmp, q->st.curr_frame);
		unshare_cell(&tmp);
		if (ok != true) return ok;
