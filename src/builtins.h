...
#define eval(q,c)														\
	...
	if (q->did_throw)													\
		return true; 												\
	if (is_var(c))													\
		return throw_error(q, c, q->st.cur_frame, "instantiation_error", "number"); \
	if (is_builtin(c) && c->bif_ptr && (c->bif_ptr->fn != bif_iso_float_1) && (c->bif_ptr->fn != bif_iso_integer_1)) \
		return throw_error(q, c, q->st.cur_frame, "type_error", "evaluable");
...
#define GET_CURR_FRAME() GET_FRAME(q->st.cur_frame)
...
inline static cell *get_first_arg(query *q)
{
	...
	return deref(q, q->last_arg, q->st.cur_frame);
}
...
#define checked(expr, ...) \
	...
	return throw_error(q, q->st.instr, q->st.cur_frame, "resource_error", "memory"))
...
#define START_FUNCTION(q) { \
	...
	if (!q->eval) { \
		if (!q->st.m->flags.unknown) \
			return throw_error(q, q->st.instr, q->st.cur_frame, "existence_error", "procedure"); \
		else \
			return false; \
	} \
}
