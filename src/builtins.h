...
#define is_character(c)                                                                            \
    (is_iso_atom(c) &&                                                                             \
     ((strlen_utf8(C_STR(q, c)) <= 1) || !CMP_STRING_TO_CSTR(q, c, "end_of_file")))
#define is_character_or_var(c) (is_in_character(c) || is_var(c))
#define is_in_character(c)                                                                         \
    (is_atom(c) && ((strlen_utf8(C_STR(q, c)) <= 1) || !CMP_STRING_TO_CSTR(q, c, "end_of_file")))
...
#define is_chars(q, c, ctx)                                                                        \
    (is_nil(c) || is_string(c) || scan_is_chars_list(q, c, ctx, false) ||                          \
     (is_cstring(c) && !CMP_STRING_TO_CSTR(q, c, "[]")))
...
#define eval(q, c)                                                                                 \
    ...
    : is_callable(c)                 ? (call_userfun(q, c, c##_ctx), q->accum)                     \
                                     : *c;                                                         \
    q->accum.flags = 0;                                                                            \
    q->accum.num_cells = 1;                                                                        \
    if (q->did_throw)                                                                              \
    ...
    if (is_builtin(c) && c->bif_ptr && (c->bif_ptr->fn != bif_iso_float_1) &&                      \
        (c->bif_ptr->fn != bif_iso_integer_1))                                                     \
...
{
    return var_num < f->initial_slots ? q->slots + f->base + var_num
...
#define GET_FIRST_ARG(p, vt)                                                                       \
    ...
    pl_ctx p##_ctx = q->latest_ctx;                                                                \
    if (!is_##vt(p)) {                                                                             \
        return q->did_throw ? false : throw_error(q, p, p##_ctx, "type_error", #vt);               \
    }
...
#define CHECK_SENTINEL(expr, err_sentinel, ...)                                                    \
    CHECK_SENTINEL_((expr), err_sentinel, ##__VA_ARGS__, error = true)
#define CHECK_SENTINEL_(expr, err_sentinel, on_error, ...)                                         \
    do {                                                                                           \
        if ((expr) == err_sentinel) {                                                              \
            on_error;                                                                              \
        }                                                                                          \
    } while (0)
