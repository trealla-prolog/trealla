...
static bool bif_bb_b_put_2(query *q)
{
	...
	if (!unify(q, &c, q->st.curr_ctx, &v, q->st.curr_ctx))
		return false;
...
static bool bif_bb_get_2(query *q)
{
	...
	if (DO_DUMP) DUMP_TERM2("bb_get", tmpbuf, tmp, q->st.curr_ctx, 1);

	if (is_var(p2) && is_var(tmp)) {
		const frame *f = GET_FRAME(q->st.curr_ctx);
...
static bool bif_bb_delete_2(query *q)
{
	...
	if (DO_DUMP) DUMP_TERM2("bb_delete", tmpbuf, tmp, q->st.curr_ctx, 1);
