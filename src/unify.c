...
static int compare_internal(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx,
                            unsigned depth);
...
static int compare_structs(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx,
                           unsigned depth)
{
                            ...
                            unsigned depth)
{
    ...
    if ((is_string(p1) && is_iso_list(p2)) || (is_string(p2) && is_iso_list(p1))) {
        LIST_HANDLER(p1);
        LIST_HANDLER(p2);
...
static void set_var(query *q, const cell *c, pl_ctx c_ctx, cell *v, pl_ctx v_ctx)
{
    ...
    if (is_var(v)) {
        ...
        if ((v_ctx >= q->st.cur_ctx) && !is_ground(v)) {
            q->no_recov = true;
            q->total_no_recovs++;
        }
    } else {
...
static bool unify_internal(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx,
                           unsigned depth);

static bool unify_string_to_list(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx,
                                 unsigned depth)
{
    LIST_HANDLER(p1);
    LIST_HANDLER(p2);
...
static bool unify_integers(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx,
                           unsigned depth)
{
    if (is_bigint(p1)) {
        if (is_rational(p2)) {
...
static bool unify_rationals(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx,
                            unsigned depth)
{
    if (is_rational(p1) && is_rational(p2))
        return !mp_rat_compare(&p1->val_bigint->irat, &p2->val_bigint->irat);
...
static bool unify_cstrings(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx,
                           unsigned depth)
{
...
static bool unify_structs(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx,
                          unsigned depth)
{
    if (p1->arity != p2->arity)
        return false;
...
static bool unify_interned(query *q, cell *p1, pl_ctx p1_ctx, cell *p2, pl_ctx p2_ctx,
                           unsigned depth)
{
    if (is_iso_list(p1) && is_iso_list(p2))
...
static const struct dispatch g_disp[] = {{TAG_EMPTY, NULL},
                                         {TAG_VAR, NULL},
