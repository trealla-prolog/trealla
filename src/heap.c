...
#define deep_copy(c) \
	(!q->noderef || (is_ref(c) && (c->val_ctx <= q->st.curr_ctx) && !is_anon(c)))
	...
	for (unsigned i = 0; i < num_cells; i++, c++) {
		...
		if (from && (c->var_num == from->var_num) && (c->val_ctx == from_ctx)) {
			...
			if (copy_attrs && attrs) {
				...
				if (!c->tmp_attrs) {
					cell *tmp =
						from ?copy_term_to_heap_with_replacement(q, attrs, q->st.curr_ctx, false, from, from_ctx, to, to_ctx)
						:copy_term_to_heap(q, attrs, q->st.curr_ctx, false);
					CHECKED(tmp);
					c->tmp_attrs = tmp;
				}
