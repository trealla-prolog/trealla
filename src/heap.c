...
cell *alloc_tmp(query *q, unsigned num_cells)
{
    ...
    if (new_size >= q->tmph_size) {
        size_t elements =
            alloc_grow(q, (void **)&q->tmp_heap, sizeof(cell), new_size, new_size * 5 / 4);
        if (!elements)
            return NULL;
...
#define deep_copy(c) (!q->noderef || (is_ref(c) && (c->val_ctx <= q->st.cur_ctx) && !is_anon(c)))
...
static cell *clone_term_to_tmp_internal(query *q, cell *p1, pl_ctx p1_ctx, unsigned depth)
{
    ...
    if (is_iso_list(p1)) {
        ...
        while (is_iso_list(p1)) {
            ...
            both = 0;
            if (deep_copy(t))
                DEREF_CHECKED(any2, both, save_vgen, e, e->vgen, t, t_ctx, q->vgen);
...
static bool copy_vars(query *q, cell *c, bool copy_attrs, cell *from, pl_ctx from_ctx, cell *to,
                      pl_ctx to_ctx)
{
    ...
    for (unsigned i = 0; i < num_cells; i++, c++) {
        ...
        if (from && (c->var_num == from->var_num) && (c->val_ctx == from_ctx)) {
            ...
            if (copy_attrs && attrs) {
                ...
                if (!c->tmp_attrs) {
                    cell *tmp =
                        from ? copy_term_to_heap_with_replacement(q, attrs, q->st.cur_ctx, false,
...
static cell *copy_term_to_tmp_with_replacement(query *q, cell *p1, pl_ctx p1_ctx, bool copy_attrs,
                                               cell *from, pl_ctx from_ctx, cell *to, pl_ctx to_ctx)
{
...
cell *copy_term_to_heap_with_replacement(query *q, cell *p1, pl_ctx p1_ctx, bool copy_attrs,
                                         cell *from, pl_ctx from_ctx, cell *to, pl_ctx to_ctx)
{
    if (!init_tmp_heap(q))
        return NULL;
    ...
    cell *tmp =
        copy_term_to_tmp_with_replacement(q, p1, p1_ctx, copy_attrs, is_var(from) ? from : NULL,
                                          from_ctx, is_var(to) ? to : NULL, to_ctx);
    if (!tmp)
        return tmp;
    cell *tmp2 = alloc_heap(q, tmp->num_cells);
    if (!tmp2)
