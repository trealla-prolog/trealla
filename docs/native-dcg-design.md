...
test, on v3.1.28 or on this tree as noted.

**Status.** Phases 0, 1, 3 and most of 5 have landed; §12 records what each one
found. This remains the *design*, not a description of the shipped code, and the
two drift where the implementation was clearer:

* §3's Layer 1 sketch names `dcg_translate_body` / `dcg_translate_rule` and a
  `newvar` callback in the context. What shipped is `xlate_body()` /
  `xlate_rule()`, with `query *q` and `parser *p` fields in `dcg_ctx` and the
  path chosen by which one is non-NULL. The consult-time entry point is
  `dcg_expand_clause()`, declared in `src/parser.h`.
* The three-outcome return code of §3 is real in the type, but Layer 1 never
  returns `DCG_DECLINE`: the decline decision is `dcg_is_constr()`, tested in
  `'$dcg_body'/4` before translation starts. The `DCG_DECLINE` branches in both
  bifs are unreachable.

`src/bif_dcgs.c`, `src/parser.c` and `library/dcgs.pl` cite this file by section
number — §1.1, §5.1, §5.2, §5.3, §6, §9, §10, §11. Renumbering breaks those.
...
**A note for users porting code: SWI and Scryer disagree here, and Trealla
follows Scryer.** ISO 7.14.11 and 7.14.12 make the existence of both constructs
implementation-defined. SWI's `boot/dcg.pl` translates `\+ G` to
`(\+ G'(S0,_), S0 = S)` and accepts `->` and `*->` at top level; Scryer's
`dcg_constr/1` throws for both, and the matching `dcg_cbody/4` clause for `\+` is
present but commented out. Preserving the reference's behaviour therefore means
matching Scryer, not "matching other systems". Open question 5 in §13 records
what happened when the SWI reading was tried.
...
**[landed, and not as written]** The var case is as described — case 1 emits
`phrase(Var,S0,S)` and never throws. The partial terminal list is not: rather
than emitting a `must_be/2` goal, `emit_terminals()` raises
`error(instantiation_error, must_be/2)` on the spot, and **the deferral belongs
to the caller**. `library/dcgs.pl`'s `user:goal_expansion` wraps the translation
in a `catch/3` and, on any throw, *declines the hook* — leaving the ordinary
`phrase/3` call in the clause to be translated again at runtime, by which point
the tail may be bound.

Two consequences worth stating, because reading `bif_dcgs.c` alone suggests the
deferral is missing:

* It is broader than `error_goal/2`, which defers `instantiation_error` and
  rethrows everything else. Trealla's hook declines on *any* throw, which is
  what §5.3's "compile-time expansion may not raise at a different moment than
  the runtime would" argues for anyway.
* It only covers `phrase/N` call sites. A `-->` rule goes through
  `'$dcg_rule'/2` via `dcg_expand_clause()`, where a partial terminal list is a
  consult-time error, as it is in the reference's `term_expansion/2` path.
...
2. **`assign_vars()` — the question was mis-framed.** It builds the vartab rather
   than requiring it. This originally read "option (b), with an ordering caveat":
   both halves were wrong. Option (b) was built and fails three ways, the answer
   is option (a), and the ordering runs the other way — translation must happen
   *before* `assign_vars()`, which is what registers the names. See §10.
...
4. **Bif registration precedes the `g_libs` consult — yes.** `prolog.c` 793 vs
   847. See §8.
5. **`\+ G` keeps the `representation_error` — tried, measured, reverted.** The
   premise this question rested on, that "quads 27–30 accept both", is **wrong**,
   and that is what settled it.

   Translating is small: `xlate_body()` case 11 recurses instead of calling
   `set_ball_repr()`, and `dcg_is_constr()` already answers true for `\+`/1, so
   `phrase/3` needs no change. SWI expands the argument at translate time rather
   than deferring through `phrase/3`, giving `(\+ G'(S0,_S1), S0 = S)`; a
   variable argument still comes back as `phrase/3` via case 1, so the two forms
   agree observably. Built, it passed the full suite.

   What it costs is conformance. Once `\+` is translated its argument reaches
   non-terminal position, where §5.3's eager check applies, so
   `phrase(([a],\+1),[])` raises `type_error(callable,1)` — and quads 29 and 30
   accept only `representation_error(dcg_body)` or `false`/`true`. That is **two
   new deviations from `phrase_quad.txt`**, not a relocation of the #1102 one,
   taking the gate from 58/58 back to 56/58. SWI answers both the same way for
   the same reason, so this is a fork inside the standard's
   implementation-defined space, not a defect on either side.

   Two smaller costs, so a later attempt does not rediscover them: §9's
   differential harness needs a divergence entry whose native side *succeeds*,
   with the emitted clause pinned by `variant/2` (every existing entry is a
   formal error term); and it leaves Trealla in neither camp anyway, since a
   bare `->` would still raise while SWI accepts `->` and `*->`.
